...
import {getFactoryTokens, getTokenFactoryParams, resetAllTokensCache, sortAssets, Token} from "@/services";
    ...
    return (
        <Box m='$6' width={'$full'}>
            <TextField
                ...
                label="Denomination"
                onChange={(e) => {
                    onFormDenomChange(e.target.value)
                }}
            ...
            <Callout
                ...
                title="Denomination"
            >
                This is used to create a base denomination for your token. Once the token is created, you can mint new
                coins, burn existing ones, transfer ownership, or modify your token’s details. A fee of {props.fee} is
                paid when a token is created.
            </Callout>
        ...
        <Box mx={{desktop: '$12', mobile: '$6'}} mb={{desktop: '$0', mobile: '$12'}}>
            <DefaultBorderedBox>
                <Box display={'flex'} flexDirection={'column'} alignItems='center'>
                    <Box p='$6' mt='$6'>
                        <Text fontSize={'$md'} fontWeight={'$bold'} color='$primary200'>Unleash Your Creativity with BZE
                            Blockchain&apos;s Token Factory!</Text>
                    </Box>
                    <Box p='$6' mb='$6'>
                        <Text letterSpacing={'$wide'} fontSize={'$sm'}>Create your own custom tokens with the Token
                            Factory module on the BZE blockchain! Experience the freedom to design denominations that
                            operate just like any other coin on the blockchain. Enjoy the flexibility of sending and
                            using your tokens both within the BZE ecosystem and across other blockchains through
                            Inter-Blockchain Communication (IBC).</Text>
                    </Box>
                </Box>
                <Divider/>
                <Box display={'flex'} m='$6' justifyContent={'center'} alignItems={'center'} flexDirection={'column'}>
                    {showForm && walletStatus === WalletStatus.Connected && <CreateTokenForm props={{
                        fee: fee, onCancel: () => {
                            setShowForm(false)
