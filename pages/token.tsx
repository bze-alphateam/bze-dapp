            ...
            <Box p='$6' mb='$6'>
                ...
                <Box mt='$4'>
                    <Text fontSize={'$md'} textAlign={'center'}
                          color={'$primary100'}>{admin !== "" ? admin : "Nobody"}</Text>
                    <Box mt='$6'>
                        <Text fontSize={'$sm'} textAlign={'center'}
                              color={admin !== "" ? '$textDanger' : '$textSuccess'}
                              fontWeight={'$thin'}>{admin !== "" ? "This address has the power to mint, burn tokens and change token metadata. Proceed with caution." : "The creator of this token gave up on the ownership. Tokens can NOT be minted anymore and the metadata can NOT be modified."}</Text>
                    </Box>
                </Box>
            </Box>
            {address === admin &&
                <>
                    ...
                    {showWarning &&
                        <Box p='$6' textAlign={"center"}>
                            ...
                            <Text fontWeight={'$light'} color={"$textDanger"}>Giving up ownership of the token is an
                                irreversible action! You will NOT be able to undo this action, mint, burn tokens or edit
                                metadata.</Text>
                        </Box>
                    }
                    <Box p='$6' flexDirection={'row'} display={'flex'} alignItems={'center'}>
                        ...
                        <Box flexDirection={'row'} display={'flex'} justifyContent={'space-around'} flex={1}
                             alignItems={'center'}>
                            {(showForm || showWarning) && <Button size="sm" intent="secondary" onClick={() => {
                                onCancelClick()
                            }} disabled={submitPending}>Cancel</Button>}
                            {showRenounceButton && <Button size="sm" intent="primary" onClick={() => {
                                onRenounceClick()
                            }} isLoading={submitPending}>Give Up Ownership</Button>}
                            {showTransferButton && <Button size="sm" intent="primary" onClick={() => {
                                onTransferClick()
                            }} isLoading={submitPending}>Transfer Ownership</Button>}
                        </Box>
                    </Box>
                </>
            }
            ...
            {address === props.admin &&
                <>
                    ...
                    <Box p='$6' flexDirection={'row'} display={'flex'} justifyContent={'space-around'}
                        ...
                        {(!showBurnButton || !showMintButton) && <Button size="sm" intent="secondary" onClick={() => {
                            onCancelClick()
                        }} disabled={submitPending}>Cancel</Button>}
                        {showBurnButton && <Button size="sm" intent="primary" onClick={() => {
                            onBurnClick()
                        }} isLoading={submitPending} disabled={!canMint}>Burn</Button>}
                        {showMintButton && <Button size="sm" intent="primary" onClick={() => {
                            onMintClick()
                        }} isLoading={submitPending} disabled={!canMint}>Mint</Button>}
                    </Box>
                </>
            }
        </DefaultBorderedBox>
    ...
    return (
        <DefaultBorderedBox mx='$6' width={{desktop: '$containerMd', mobile: '$auto'}}>
            {address === props.admin &&
                ...
                >
                    It is recommended to add full metadata to your token. The metadata is used to display token&apos;s
            ...
            <Box p='$6'>
                <TextField
                    id="base"
                    label={
                        <TooltippedText text="Base denomination"
                                        tooltip="This is the denomination generated by the blockchain when the token was created and it can not be changed"/>
                    }
                    size="sm"
            ...
            <Box p='$6' flexDirection={{desktop: 'row', mobile: 'column'}} display={'flex'}
                 justifyContent={'space-between'} width={'$auto'}>
                <TextField
                    id="display"
                    label={
                        <TooltippedText text="Ticker/Symbol (ex: BZE)"
                                        tooltip="The ticker is usually the short name of the token."/>
                    }
                    size="sm"
                    onChange={(e) => {
                ...
                <TextField
                    id="decimals"
                    label={
                        <TooltippedText text="Decimals/Exponent (ex: 6)"
                                        tooltip="The number of decimals for the largest denomination."/>
                    }
                    size="sm"
                    onChange={(e) => {
            ...
            {!disabled &&
                <Box display={'flex'} m='$6' justifyContent={'space-evenly'} flexDirection={'row'}>
                    <Button size="sm" intent="secondary" onClick={() => {
                        onEditCancel()
                    }} disabled={pendingSubmit}>Cancel</Button>
                    <Button size="sm" intent="primary" onClick={() => {
                        onEditSave()
                    }} disabled={pendingSubmit} isLoading={pendingSubmit}>Save</Button>
                </Box>
            }
        </DefaultBorderedBox>
    );
        ...
        <Layout>
            <Box display='block' flexDirection={'row'}>
                <Box marginBottom={'$12'} ml='$6'>
                    <Text as="h1" fontSize={'$2xl'}>Token Details {chainMetadata?.name ? "/" : ""}<Text
                        fontSize={'$2xl'} color={'$primary300'} as="span"> {chainMetadata?.name}</Text></Text>
                </Box>
            </Box>
