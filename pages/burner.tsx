...
import {
    ...
    getRaffles as getBlockchainRaffles,
    getRaffleWinners,
                ...
                <Box overflowY={'scroll'} maxHeight={"30vw"} width={'$full'}>
                    {winners.length === 0 ?
                        ...
                        winners.map((winner: RaffleWinnerSDKType, index: number) => {
                            ...
                            return (
                                <Box key={index + winner.winner} as="div" p='$2' display={'flex'} flexDirection={'row'}
                                     alignItems={'center'} flex={1}>
                                    <Box display={'flex'} flex={1} justifyContent={'center'}>
                                        <Box display={'flex'} flex={1} flexDirection={'row'}
                                             justifyContent={'space-between'}>
                                            <Text fontSize={'$lg'} color={textColor}
                                                  fontWeight={'$semibold'}>{stringTruncateFromCenter(winner.winner, 12)}{isMe ? ` (You)` : undefined}</Text>
                                            <Box width={'55px'}></Box>
                                            {/*@ts-ignore*/}
                                            <Text fontSize={'$lg'} color={textColor}
...
function Contribute({onContributeSuccess}: ContributeProps) {
    ...
    return (
        <Box display={'flex'} flexDirection={'column'} alignItems='center'>
            ...
            <Box p='$6'>
                <Text fontWeight={'$thin'} fontSize={'$sm'} color='$primary200'>You can fund the next burning event with
                    any amount of BZE you want to see burned.</Text>
            </Box>
            {
                showForm &&
                <Box p='$6'>
                    ...
                    >
                        This operation cannot be reversed, and coins you donate for burning cannot be recovered! Once
                        the transaction is signed, the coins are lost forever.
                    </Callout>
                </Box>
            }
...
const RaffleIntro = memo(() => {
    return (
        <>
            ...
            <Box display='flex' m='$6'>
                <Text fontSize={'$md'}>Participate in Burning Raffles and win a share of the burning pot. Users can
                    purchase a ticket to have a chance to win a percentage of the pot. Each raffle is configured at the
                    start and has its own specifications. A percentage of the pot can be won by users who place a
                    ticket. After placing a ticket, the blockchain will determine the outcome after two blocks to see if
                    it is a winning ticket. If the ticket is a winner, the blockchain will automatically transfer the
                    prize to the winning address. When the raffle ends, any remaining coins are burned.</Text>
            </Box>
        </>
    );
});
...
const RafflesBoxItem = memo((props: { raffle: RaffleBoxRaffle }) => {
    ...
    return (
        <DefaultBorderedBox p='$2' m='$2' flex='1' key={props.raffle.sdk.denom}>
            <Box flex={1} display={'flex'} flexDirection={{'mobile': 'column', desktop: 'row'}}
                 alignItems={{mobile: 'center', desktop: 'center'}}
                 justifyContent={{mobile: undefined, desktop: 'space-between'}}>
                <Box p='$6'>
                    <Text fontSize={'$lg'} fontWeight={'$bold'}
                          color='$primary300'>{props.raffle.displayDenom.denom.toUpperCase()} Raffle</Text>
                </Box>
                <Box p='$6'>
                    <Text fontSize={'$lg'} color='$primary300'> Ticket
                        price: {ticketPrice} {props.raffle.displayDenom.denom.toUpperCase()}</Text>
                </Box>
                <Box p='$6'>
            ...
            <Divider/>
            <Box flex={1} display={'flex'} flexDirection={{'mobile': 'column-reverse', desktop: 'row'}}
                 justifyContent={'space-evenly'}>
                <Box>
                    <Box minWidth={{mobile: '$auto', desktop: '500px'}} flex={1} display={'flex'}
                         flexDirection={'column'} justifyContent={'space-between'}>
                        <RaffleDescriptionTextBox text={'Winners'} value={props.raffle.sdk.winners.toString()}/>
                        <RaffleDescriptionTextBox text={'Total won'}
                                                  value={`${uAmountToAmount(props.raffle.sdk.total_won, props.raffle.displayDenom.exponent)} ${props.raffle.displayDenom.denom.toUpperCase()}`}/>
                    ...
                    <Box flex={1} display={'flex'} flexDirection={{'mobile': 'column', desktop: 'row'}}
                        ...
                        <Box p='$6'>
                            <Button size="sm" intent="secondary"
                                    onClick={() => props.raffle.onWinnersClick(props.raffle)}>View winners</Button>
                        </Box>
                    </Box>
                </Box>
                <DefaultBorderedBox flex={1} display={'flex'} m={'$6'} p={'$6'} boxShadow={undefined}
                                    flexDirection={'column'} alignItems={'center'} justifyContent={'center'}>
                    <Box p='$6'>
                        <Text fontSize={'$lg'} fontWeight={'$bold'} color='$primary300'>Current Prize:
                            ~{calculateCurrentPrize(props.raffle)}</Text>
                    </Box>
                    <Box p='$6'>
                        <Text fontSize={'$sm'} color='$primary200' fontWeight={'$hairline'}>You will
                            pay {ticketPrice} {props.raffle.displayDenom.denom.toUpperCase()} to try your luck.</Text>
                    </Box>
                    <Box p='$6'>
                        <Button size="sm" intent="primary" disabled={(remaining === undefined) || pending}
                                isLoading={pending || waitingResult} onClick={() => submitTicket(props.raffle)}>Try your
                            luck</Button>
                    </Box>
                    <Box p='$2'>
                        {pending && !waitingResult && <Text fontSize={'$sm'} fontWeight={"$light"} color='$green200'>Submitting
                            transaction...</Text>}
                        {!pending && waitingResult && <Text fontSize={'$sm'} fontWeight={"$light"}
                                                            color='$green200'>{"Let's see how lucky you are..."} ({waitingSeconds}s)</Text>}
                        {!pending && !waitingResult && withdrawResult && !withdrawResult.hasWon &&
                            <Text fontSize={'$sm'} fontWeight={"$light"} color='$red200'>Unlucky! 😔 Better luck next
                                time!</Text>}
                        {!pending && !waitingResult && withdrawResult?.hasWon &&
                            <Text fontSize={'$sm'} fontWeight={"$light"} color='$green200'>You
                                won {uAmountToAmount(withdrawResult.amount, props.raffle.displayDenom.exponent)} {props.raffle.displayDenom.denom.toUpperCase()}!
                                🥳</Text>}
                    </Box>
                </DefaultBorderedBox>
            </Box>
        ...
        <BurnBox title='Raffles'>
            ...
            <Box p='$2' m='$6' flex='1'>
                <Box display={'flex'} flexDirection={'column'} alignItems='stretch'>
                    {raffles.length === 0 ?
                        (<Box flex={1} display={'flex'} justifyContent={'center'}><Text>No active raffles found. Try
                            again later...</Text></Box>) :
                        raffles.map((raff) => (
