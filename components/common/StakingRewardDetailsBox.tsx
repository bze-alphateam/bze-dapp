...
export function StakingRewardDetailsBox({children, ...props}: StakingRewardDetailsBoxProps) {
    const {
        ...
        status,
        statusColor
    } = useMemo((): StakingRewardDetailMemoData => {
    ...
    return (
        <DefaultBorderedBox m='$6' p='$6' flexDirection={'column'}>
            <Box display={'flex'} flex={1} flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'}
                ...
                <Box>
                    ...
                    <Text fontWeight={'$bold'} fontSize={'$lg'}
                          color={'$primary200'}>{props.stakingToken.metadata.display.toUpperCase()}</Text>
                </Box>
                <Box>
                ...
                <Box>
                    ...
                    <Text fontWeight={'$bold'} fontSize={'$lg'}
                          color={'$primary200'}>{props.prizeToken.metadata.display.toUpperCase()}</Text>
                </Box>
            </Box>
            ...
            <StakingRewardDetailsBoxRow props={{
                name: 'Verified:',
                value: props.prizeToken.verified && props.stakingToken.verified ? '✅ YES' : '❌ NO'
            }}/>
            {children}
        </DefaultBorderedBox>
    );
